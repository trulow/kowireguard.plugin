...
function Menu.statusText()
    ...
    -- A looping tunnel looks perfectly healthy on every other measure --
    -- interface up, process alive, peer present, handshake recent -- so this
    -- has to lead.  Appended at the bottom of a five-line status it renders
    -- off the end of a truncated menu item and is never seen.
    if st.endpoint_looping then
        local parts = {
            _("Status: PROBLEM — endpoint exclusion lost"),
            _("Traffic is looping inside the tunnel. Repairing…"),
            T(_("Interface: %1"), st.iface),
        }
        if st.rx and st.tx then
    ...
    if st.paused then
        return _("Status: paused — waiting for the network") .. "\n" ..
            _("The interface is down so traffic cannot loop. Repairing…")
    end
