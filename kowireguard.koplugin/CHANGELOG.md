...
- **`ip: invalid argument '51820' to 'table'`** — connect failed at the policy
  route step. BusyBox's `ip` rejects routing table IDs above 255 and has no
  `/etc/iproute2/rt_tables` to name higher ones. The table was chosen as
  WireGuard's port number for cosmetic reasons, without checking; it is now
  100, which is inside the valid range, outside the reserved IDs (253–255),
  and is the value the on-device test used.

### Changed

- Teardown deletes only the plugin's own default route from the table rather
  than flushing it. Nothing else on this device uses table 100, but flushing a
  shared table is the same class of mistake as flushing the firewall.
...
- **The endpoint exclusion now uses policy routing instead of a host route.**
  ...
  20 MB/s on this device. Previous versions arranged that with a host route
  for the endpoint address via the physical gateway.

  That works, but it excludes the address for *all* traffic. Any other
  service reachable at the same IP — common when a WireGuard endpoint and a
  web service sit behind one public address — became unreachable through the
  tunnel, arriving instead from whatever network the device was on.

  The plugin now sets an fwmark on the WireGuard interface and adds an
  `ip rule` sending marked packets out the physical interface via a dedicated
  routing table. The exclusion applies only to WireGuard's own traffic;
  everything else, including requests to the endpoint's address, routes
  through the tunnel normally.

  This is what `wg-quick` does with `Table = auto`. It should have been the
  approach from the start.

  Verified on device: with the fwmark set, `ip route get <endpoint>` returns
  `dev wg0` while the handshake stays healthy and the counters stay flat, and
  a service sharing the endpoint's public IP returns 200 through the tunnel.

- **The safety gate changed accordingly.** Previously the plugin verified the
  endpoint host route with `ip route get` before bringing the interface up.
  It now verifies that the fwmark actually applied, and that the policy rule
  and table resolve through the physical interface, aborting if either fails.
  This gate is load-bearing: without a working fwmark, installing the
  split-default routes starts the loop with nothing to stop it.

- Resume re-verifies the policy rule rather than the host route; the loop
  detector watches for a missing rule; startup reconciliation treats a
  leftover rule as state to clean up.

- Diagnostics → Routing now shows `ip rule` and the dedicated table alongside
  the main routing table.

### Removed

- `Net.addEndpointRoute`, `Net.verifyEndpointRoute`, `Net.delEndpointRoute`.

### Note

This supersedes the `#Host` override mechanism explored in 1.1.0/1.2.0, which
worked around the host-route limitation by rewriting `/etc/hosts`. With
policy routing the workaround is unnecessary — nothing is written to `/etc`,
so the read-only rootfs on this device is no longer a factor.
...
[2.0.1]: #201--2026-09-03
[2.0.0]: #200--2026-09-03
