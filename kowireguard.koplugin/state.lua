...
function State.status()
    ...
    -- Endpoint route drift.  If the endpoint has fallen into the tunnel the
    -- connection looks healthy -- interface up, process alive, peer present --
    -- while wireguard-go re-encrypts its own output in a loop.  Surface it.
    local j = State.readJournal()
    if j and j.paused then
        st.paused = true
    elseif j and j.gw and j.dev then
        -- Without the policy rule, WireGuard's own packets re-enter the
        -- tunnel and the interface encrypts its own output in a loop.  The
        -- connection looks healthy on every other measure, so surface it.
        if not Net.markRouteVerify(j.gw, j.dev) then
            st.endpoint_looping = true
        end
    end
