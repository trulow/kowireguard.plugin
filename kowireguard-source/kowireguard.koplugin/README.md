...
Restart KOReader. The plugin appears under **Network → kowireguard (v1.0.3)**.
...
so the in-place `updateItems()` calls in this plugin's menu code are no-ops on
that version. In practice the status line updates **when you open the menu,
not while it is open** — `text_func` is re-evaluated on every open, so backing
out and reopening always shows current state.

Everything else works normally: connect, disconnect and settings changes all
take effect immediately, only the live redraw is inert.
