...
-- How many times, and how often, to retry a resume repair that could not be
-- completed on the first attempt.  Kindle Wi-Fi can take a while to
-- reassociate after a deep sleep, and until it does there is no default route
-- to pin the endpoint route to.  Roughly 3, 6, 9 ... seconds, giving about a
-- minute in total.
local RESUME_RETRY_LIMIT = 8
local RESUME_RETRY_STEP = 3

function KOWG:scheduleReassert(attempt)
    attempt = attempt or 1
    UIManager:scheduleIn(attempt == 1 and 2 or RESUME_RETRY_STEP, function()
        ...
        if why == "retry" then
            if attempt < RESUME_RETRY_LIMIT then
                self:scheduleReassert(attempt + 1)
            else
                -- Give up rather than leave the tunnel looping on itself
                -- indefinitely.  A torn-down tunnel is honest; one that
                -- silently re-encrypts its own output is not.
                Util.log("resume: could not repair after", RESUME_RETRY_LIMIT,
                    "attempts, tearing down to stop the tunnel looping")
                env.tunnel.disconnect()
                UIManager:show(InfoMessage:new{
                    text = _("kowireguard disconnected: the network did not come back after resume."),
                    timeout = 5,
                })
            end
        end
