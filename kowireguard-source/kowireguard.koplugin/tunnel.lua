...
function Tunnel.connect(conf_name)
    ...
    -- 5. Endpoint host route, then verify it.  Never proceed unverified.
    local okr, rerr = Net.addEndpointRoute(ep_ip, dr.gw, dr.dev)
    if not okr then return fail(journal, rerr) end
    mark("endpoint_route")

    local okv, verr = Net.verifyEndpointRoute(ep_ip, dr.gw, dr.dev)
    if not okv then
        return fail(journal, verr .. " " .. _("Refusing to bring the tunnel up."))
    end
    Util.log("endpoint route verified:", ep_ip, "via", dr.gw, "dev", dr.dev)
...
function Tunnel.disconnect()
    ...
    if j.endpoint_ip then Net.delEndpointRoute(j.endpoint_ip) end
...
function Tunnel.reassert()
    ...
    if j.endpoint_ip and j.gw and j.dev then
        local ok = Net.verifyEndpointRoute(j.endpoint_ip, j.gw, j.dev)
        if not ok then
            local dr, drerr = Net.defaultRoute()
            if dr then
                local added = Net.addEndpointRoute(j.endpoint_ip, dr.gw, dr.dev)
                if added and Net.verifyEndpointRoute(j.endpoint_ip, dr.gw, dr.dev) then
                    j.gw, j.dev = dr.gw, dr.dev
                    Tunnel.writeJournal(j)
                    repaired[#repaired + 1] = "endpoint route"
                else
                    retry = true
                    Util.log("resume: endpoint route re-added but did not verify")
                end
            else
                retry = true
                Util.log("resume: endpoint route is missing and cannot be repaired yet",
                    "(" .. tostring(drerr) .. ")")
            end
        end
    end
